...
ClassicGDParameters = namedtuple(
    'ClassicGDParameters',
    ['learning_rate', 'exploration_step_size', 'n_random_steps', 'n_iteration',
     'stop_criterion', 'seed'])
ClassicGDParameters.__doc__ = """
:param learning_rate: The rate of learning per step of gradient descent
:param exploration_step_size: The standard deviation of random steps used for
...
StochasticGDParameters = namedtuple(
    ...
    ['learning_rate', 'stochastic_deviation', 'stochastic_decay',
     'exploration_step_size', 'n_random_steps', 'n_iteration',
     'stop_criterion', 'seed'])
StochasticGDParameters.__doc__ = """
:param learning_rate: The rate of learning per step of gradient descent
:param stochastic_deviation: The standard deviation of the random vector used
 to perturbate the gradient
:param stochastic_decay: The decay of the influence of the random vector that
 is added to the gradient (set to 0 to disable stochastic perturbation)
:param exploration_step_size: The standard deviation of random steps used for
 finite difference gradient
:param n_random_steps: The amount of random steps used to estimate gradient
:param n_iteration: number of iteration to perform
...
AdamParameters = namedtuple(
    'AdamParameters',
    ['learning_rate', 'exploration_step_size', 'n_random_steps',
     'first_order_decay', 'second_order_decay', 'n_iteration',
     'stop_criterion', 'seed'])
...
:param exploration_step_size: The standard deviation of random steps used for
 finite difference gradient
:param n_random_steps: The amount of random steps used to estimate gradient
:param first_order_decay: Specifies the amount of decay of the historic first
 order momentum per gradient descent step
:param second_order_decay: Specifies the amount of decay of the historic
 second order momentum per gradient descent step
:param n_iteration: number of iteration to perform
:param stop_criterion: Stop if change in fitness is below this value
...
RMSPropParameters = namedtuple(
    'RMSPropParameters',
    ['learning_rate', 'exploration_step_size', 'n_random_steps',
     'momentum_decay', 'n_iteration', 'stop_criterion', 'seed'])
RMSPropParameters.__doc__ = """
:param learning_rate: The rate of learning per step of gradient descent
:param exploration_step_size: The standard deviation of random steps used for
 finite difference gradient
:param n_random_steps: The amount of random steps used to estimate gradient
:param momentum_decay: Specifies the decay of the historic momentum at each
...
class GradientDescentOptimizer(Optimizer):
    ...
    For n iterations do:
        - Explore the fitness of individuals in the close vicinity of the
          current one
        - Calculate the gradient based on these fitnesses.
        - Create the new 'current individual' by taking a step in the
          parameters space along the direction of the largest ascent

    NOTE: This expects all parameters of the system to be of floating point

    :param  ~l2l.utils.trajectory.Trajectory traj:
      Use this trajectory to store the parameters of the specific runs. The
      parameters should be initialized based on the values in `parameters`
    
    :param optimizee_create_individual:
      Function that creates a new individual
    
    :param optimizee_fitness_weights: 
      Fitness weights. The fitness returned by the Optimizee is multiplied by
      these values (one for each element of the fitness vector)
    
    :param parameters: 
      Instance of :func:`~collections.namedtuple`
      :class:`.ClassicGDParameters`,
      ...
      parameters needed by the Optimizer. The type of this parameter is used
      to select one of the GD variants.
    ...
    def __init__(self, traj,
        ...
        if optimizee_bounding_func is not None:
            new_individual_list = [self.optimizee_bounding_func(ind)
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        logger.info(
            "-- End of iteration {}, current fitness is {} --".format(
    ...
    def init_classic_gd(self, parameters, traj):
        ...
        :param parameters: Not used - not parametrized other than
        external learn rate
        :param ~l2l.utils.trajectory.Trajectory traj: The  trajectory on which
         the parameters should get stored.
    ...
    def init_rmsprop(self, parameters, traj):
        ...
        :param parameters: should contain momentum_decay
        :param ~l2l.utils.trajectory.Trajectory traj: The  trajectory on
         which the parameters should get stored.
        ...
        traj.f_add_parameter('momentum_decay', parameters.momentum_decay,
                             comment='Decay of the historic momentum at'
                                     ' each gradient descent step')

        self.delta = 10 ** (-6)  # used to for numerical stabilization
        self.so_moment = np.zeros(
            len(self.current_individual))  # second order moment

    def init_adam(self, parameters, traj):
        """
        ADAM specific initializiation.
        :param parameters: should contain first_order_decay and
        second_order_decay ?rates?
        :param ~l2l.utils.trajectory.Trajectory traj: The  trajectory on
         which the parameters should get stored.
        ...
        traj.f_add_parameter('second_order_decay',
                             parameters.second_order_decay,
                             comment='Decay of the second order momentum')

        self.delta = 10 ** (-8)  # used for numerical stabilization
        self.fo_moment = np.zeros(
            len(self.current_individual))  # first order moment
        self.so_moment = np.zeros(
            len(self.current_individual))  # second order moment
    ...
    def init_stochastic_gd(self, parameters, traj):
        ...
        :param parameters: should contain standard deviation and decay
        :param ~l2l.utils.trajectory.Trajectory traj: The trajectory in
         which the parameters should get stored.
        ...
        traj.f_add_parameter('stochastic_deviation',
                             parameters.stochastic_deviation,
                             comment='Standard deviation of the random '
                                     'vector added to the gradient')
        traj.f_add_parameter('stochastic_decay', parameters.stochastic_decay,
                             comment='Decay of the random vector')

    def classic_gd_update(self, traj, gradient):
        """
        Updates the current individual using the classic Gradient Descent
         algorithm.

        :param ~l2l.utils.trajectory.Trajectory traj: The  trajectory which
         contains the parameters required by the update algorithm

        :param ~numpy.ndarray gradient: The gradient of the fitness curve,
         evaluated at the current individual
    ...
    def rmsprop_update(self, traj, gradient):
        ...
        self.so_moment = (traj.momentum_decay * self.so_moment +
                          (1 - traj.momentum_decay) * np.multiply(gradient,
                                                                  gradient))
        self.current_individual += np.multiply(
            traj.learning_rate / (np.sqrt(self.so_moment + self.delta)),
            gradient)
    ...
    def adam_update(self, traj, gradient):
        ...
        self.so_moment = (traj.second_order_decay * self.so_moment +
                          (1 - traj.second_order_decay) *
                          np.multiply(gradient, gradient))
        fo_moment_corrected = \
            self.fo_moment / (1 - traj.first_order_decay ** (self.g + 1))
        so_moment_corrected =\
            self.so_moment / (1 - traj.second_order_decay ** (self.g + 1))

        self.current_individual += \
            traj.learning_rate * fo_moment_corrected / \
            (np.sqrt(so_moment_corrected) + self.delta)

    def stochastic_gd_update(self, traj, gradient):
        """
        Updates the current individual using a stochastic version of the
        ...
        gradient += (self.random_state.normal(0.0, traj.stochastic_deviation,
                                              self.current_individual.size) *
