...
class EvolutionStrategiesOptimizer(Optimizer):
    ...
    as in: Salimans, T., Ho, J., Chen, X. & Sutskever, I. Evolution Strategies
           as a Scalable Alternative to Reinforcement Learning.
           arXiv:1703.03864 [cs, stat] (2017).

    In the pseudo code the algorithm does:

    For n iterations do:
      - Perturb the current individual by adding a value with 0 mean and
        `noise_std` standard deviation
      - If mirrored sampling is enabled, also perturb the current individual
        by subtracting the same values that were added in the previous step
      ...
      - If fitness shaping is enabled, F_i is replaced with the utility u_i in
        the previous step, which is calculated as:

            u_i = max(0, log(n/2 + 1) - log(k)) /
                         sum_{k=1}^{n}{max(0, log(n/2 + 1) - log(k))} - 1 / n

        As in the paper: Wierstra, D. et al. Natural Evolution Strategies.
        Journal of Machine Learning Research 15, 949–980 (2014).

        where k and i are the indices of the individuals in descending order
        of fitness F_i



    NOTE: This is not the most efficient implementation in terms of
    communication, since the new parameters are communicated to the individuals
    rather than the seed as in the paper.
    ...
    :param optimizee_create_individual:
      Function that creates a new individual. All parameters of the
      Individual-Dict returned should be of numpy.float64 type
    ...
    :param parameters:
      ...
      :class:`.CrossEntropyParameters` containing the
    ...
    def __init__(self,
        ...
        traj.f_add_parameter('noise_std', parameters.noise_std,
