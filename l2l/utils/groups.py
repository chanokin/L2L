...
class ParameterGroup:
    """
    This class is a Dictionary which can be used to store parameters. It is
    used to fit the pypet already existing interface with the trajectory
    """
    ...
    def f_add_parameter(self, key, val, comment=""):
        """
        Adds parameter with name key and value val. The comment is ignored
        for the moment but kept for compatibility with the pypet groups
...
class ResultGroup(sdictm):
    """
    ResultGroup is a class derived from sdictm, which is a dictionary
    with parameters accessible using a . (dot)
    ...
    def f_add_result(self, key, val, comment=""):
        """
        Adds a result in a result group. The name of the result group precedes
        the name of the result name and they are split by a . (dot)
        In case this result is not to be part of a result group, it is added
        to the root level of the dictionary.
        :param key: the name of the result to add. Preceded by a result group
        name if it is to be added to an existing
        group. Produces an error if the value is to be added to a non existent
        result group.
        :param val: Value of the result to be added
        :param comment: Not used
        :exception: Produces an exception if the value is to be added to
        a non existent result group.
        """
        if '.' in str(key):
            subkey = key.split('.')
    ...
    def f_add_result_to_group(self, group_name, key, val, comment=""):
        ...
        :param val:
        :param comment: not used now
        :exception Produces an exception if the value is to be added to
        a non existing result group.
        """
...
class ParameterDict(sdictm):
    """
    ParameterDict is a class derived from sdictm which takes care of holding
    parameters in the trajectory
    ...
    def __getattr__(self, attr):
        """
        This function has been overwritten in order to allow a particular
        access to values in the dictionary.
        If attr is ind_idx, it returns the id from the current result with
        index trajectory.v_idx
        ...
        if '.' in attr:
            # This is triggered exclusively in the case where __getattr__
            # is called from __getitem__
            attrs = attr.split('.')
